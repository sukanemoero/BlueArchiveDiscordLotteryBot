...
{
    protected static List<uint> GetLotteryCharacterIds(ulong id, uint school)
    {
        var data = Sql.FindWithJoinTable(
            [ICharacterData.Tables.Main, IUserData.Tables.Lottery],
            new Dictionary<Database.Mysql.TableValue, object>
            {
                { IUserData.Tables.LotteryId, id },
                { ICharacterData.Tables.MainSchool, school }
            },
